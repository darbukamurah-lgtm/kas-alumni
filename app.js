...
(function initSupabase(){
  ...
  if (lib && typeof lib.createClient === "function") {
    ...
    console.log("Supabase gagal load ❌");
  }
})();
...
async function loadTransaksi(){
  console.log("Ambil transaksi...");

  // 1. Tarik Data Anggota Dulu (WAJIB!)
...
function renderDashboard(){
  ...
  transaksi.forEach(t => {
    ...
    // Logic: Hitung jika (Sukses dari Gateway) ATAU (Data lama yang tidak ada status pending/sukses)
    const isSukses = ket.includes("sukses");
    const isDataLama = !ket.includes("pending") && !ket.includes("sukses");

    if (isSukses || isDataLama) {
...
// anggota gak usah animasi (biar normal aja)
...
function updateDashboard(){
  let sudah = anggotaList.filter(a => {
    let totalBayar = transaksi
      .filter(t => {
        const ket = t.ket.toLowerCase();
        return t.nama === a.nama && 
               t.jenis === "pemasukan" && 
               (ket.includes("sukses") || (!ket.includes("pending") && !ket.includes("sukses"))) &&
               !(t.ket && t.ket.startsWith("Urunan")) &&
               t.ket !== "Pemasukan lainnya"
      })
...
function hitungTunggakan(){
  tunggakanData = anggotaList.map(a => {
    ...
    return {
      nama: a.nama,
      status: totalBayar >= 50000 ? "LUNAS" : "CICIL",
      total: totalBayar
    };
  });
